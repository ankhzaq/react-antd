...
import refusals from '../store/sagas/refusals';
import Capsule from 'components/Capsule';
import { getRefusalsDataAction } from '../store/actions/refusals';
const { Content } = Layout;

const { Option } = Select;

const GROUP_BY_COLUMNS = ['countryId', 'area', 'refusalReason', 'storageZoneType'];
...
function Hammurabi(props: any) {
  ...
  const { common, refusals } = useSelector((state: any) => state);
  const { grid: { data, loading }, filtersPanel: { data: dataFilterPanel, date: dateFilterPanel } } = refusals;

  useEffect(() => {
    console.log("Loading refusals: ", loading);
  }, [loading]);
  ...
  const [filters, setFilters] = useState<Filter>(refusals.filters, "filterGrid");
  ...
  const setNewFilters = (newFilters: any) => {
    refusals.filters = newFilters;
    setSessionStorage("refusals", refusals);
      ...
      if (firstElement) {
        ...
        const columnsResponse = columns.map((key: string) => ({
          ...
          headerRenderer: (info: any) => {
            ...
            const currentFilters: BasicObject = getSessionStorage("refusals");
            ...
            const filtersSession = getSessionStorage("refusals").filters;
...
const mapDispatchToProps = (dispatch: any) => ({
  getData: (payload: any) => dispatch(getRefusalsDataAction(payload))
});
